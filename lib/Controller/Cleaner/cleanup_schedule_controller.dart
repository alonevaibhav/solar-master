...
class CleanupScheduleController extends GetxController {
  ...
  final todayReport = <Map<String, dynamic>>[].obs;
  ...
  Future<void> fetchTodayReport(int scheduleId) async {
    try {
      ...
      if (response.success == true) {
        ...
        // Clear any existing data in todayReport
        todayReport.clear();

        // Add the fetched data to todayReport
        todayReport.add(responseData);

        // Optionally, you can also directly access and save specific fields
        final int? reportId = responseData['id'];
        final String? reportStatus = responseData['status'];

        print('Report ID: $reportId');
        print('Report Status: $reportStatus');

        // Handle the successful response
        print('Report fetched successfully: $responseData');
        Get.snackbar(
          'Success',
        ...
        print('Failed to fetch report: ${response.errorMessage}');
        Get.snackbar(
          'Error',
          'Failed to fetch report: ${response.errorMessage}',
          snackPosition: SnackPosition.BOTTOM,
          backgroundColor: Colors.red.withOpacity(0.1),
          colorText: Colors.red,
        );
