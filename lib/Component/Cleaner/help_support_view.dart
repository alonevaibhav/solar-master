...
// Updated _buildSlotItem method with visual warnings
  ...
  Widget _buildSlotItem(BuildContext context, Map<String, dynamic> slot) {
    ...
    // Check if this slot has overlap warning
    final hasOverlap = slotController.overlappingSlots.contains(slotCode);
    final conflicts = slotController.overlapDetails[slotCode] ?? [];

    return Container(
      padding: EdgeInsets.all(16),
      decoration: BoxDecoration(
        color: hasOverlap ? Colors.red[50] : Colors.grey[50],
        borderRadius: BorderRadius.circular(10),
        border: Border.all(
          color: hasOverlap
              ? Colors.red[300]!
      ...
      child: Column(
        children: [
          // Warning indicator at top if overlap exists
          if (hasOverlap)
            Container(
          ...
          Obx(() => InkWell(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                color: hasOverlap
                    ? Colors.red[100]
                    : slotController.isEditMode.value
                    ? (slotController.modifiedSlots.contains(slotCode)
                    ? Colors.orange[50]
                    : Colors.white)
                    : Colors.white,
              ...
              child: Column(
                children: [
                  Icon(
                    hasOverlap ? Icons.warning : Icons.access_time,
                    size: 16,
                    color: hasOverlap
                        ? Colors.red[600]
                        : slotController.modifiedSlots.contains(slotCode)
                    ...
                    children: [
                      ...
                      if (slotController.modifiedSlots.contains(slotCode))
                        Container(
                          ...
                          height: 6,
                          decoration: BoxDecoration(
                            color: hasOverlap ? Colors.red : Colors.orange,
                            shape: BoxShape.circle,
                          ),
                        ),
                    ],
          ...
          // Show conflict details if overlap exists
          if (hasOverlap && conflicts.isNotEmpty)
            Container(
...
// Updated header with overlap summary
  Widget _buildEditModeHeader() {
    return Obx(() {
      if (!slotController.isEditMode.value) return SizedBox();

      final hasOverlaps = slotController.overlappingSlots.isNotEmpty;

      return Container(
        width: double.infinity,
        padding: EdgeInsets.all(16),
        color: hasOverlaps ? Colors.red[50] : Colors.blue[50],
        child: Row(
          children: [
            Icon(
              hasOverlaps ? Icons.warning : Icons.edit,
              size: 20,
              color: hasOverlaps ? Colors.red[700] : Colors.blue[700],
            ),
            SizedBox(width: 8),
            Expanded(
              child: Column(
                ...
                children: [
                  Text(
                    hasOverlaps
                        ? '⚠️ Overlaps Detected - Review Before Saving'
                        : 'Edit Mode: Tap on time values to modify',
                    style: TextStyle(
                      color: hasOverlaps ? Colors.red[700] : Colors.blue[700],
                      fontWeight: FontWeight.w600,
                  ...
                  if (hasOverlaps)
                    Text(
                      '${slotController.overlappingSlots.length} slot(s) have timing conflicts',
                      style: TextStyle(
            ...
            if (slotController.modifiedSlots.isNotEmpty)
              Container(
                padding: EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                decoration: BoxDecoration(
                  color: hasOverlaps ? Colors.red[100] : Colors.orange[100],
                  borderRadius: BorderRadius.circular(12),
                ),
                child: Text(
                  '${slotController.modifiedSlots.length} modified',
                  style: TextStyle(
                    color: hasOverlaps ? Colors.red[800] : Colors.orange[800],
                    fontSize: 12,
                    fontWeight: FontWeight.w600,
                  ),
                ),
...
// Simplified time dialog - no blocking validation
  void _showKeyboardTimeDialog(BuildContext context, Map<String, dynamic> slot) {
    final slotCode = slot['code'].toString();
    final currentValueInMinutes = slotController.getCurrentSlotValue(slotCode);
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          title: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text('Edit Time', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w600)),
              SizedBox(height: 4),
              Text(
                slot['description'],
            ...
            child: Column(
              ...
              children: [
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text('Hours', style: TextStyle(fontSize: 12, color: Colors.grey[600], fontWeight: FontWeight.w500)),
                          SizedBox(height: 8),
                          TextFormField(
                            controller: hoursController,
                            keyboardType: TextInputType.number,
                    ...
                    SizedBox(width: 16),
                    Text(':', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: Colors.grey[600])),
                    SizedBox(width: 16),
                    // Minutes input
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text('Minutes', style: TextStyle(fontSize: 12, color: Colors.grey[600], fontWeight: FontWeight.w500)),
                          SizedBox(height: 8),
                          TextFormField(
                            controller: minutesController,
                            keyboardType: TextInputType.number,
                ...
                Container(
                  ...
                  child: Text(
                    'Set any time freely. Overlaps will be highlighted with warnings.',
