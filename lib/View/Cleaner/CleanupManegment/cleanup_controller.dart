...
class CleaningManagementController extends GetxController {
  ...
  final maintenanceETA = 120.obs; // Default 120 minutes
  final isMaintenanceModeEnabled = false.obs;
  final isEtaTimerActive = false.obs;
  ...
  Future<void> _saveEtaTimer(int taskId, int remainingMinutes) async {
    final currentTime = DateTime.now().millisecondsSinceEpoch;
    await _prefs.setInt('eta_task_id_$taskId', taskId);
    await _prefs.setInt('eta_remaining_minutes_$taskId', remainingMinutes);
    await _prefs.setInt('eta_start_time_$taskId', currentTime);
    await _prefs.setBool('eta_active_$taskId', true);
  }

  Future<void> _loadEtaTimer(int taskId) async {
    final savedTaskId = _prefs.getInt('eta_task_id_$taskId');
    final savedRemainingMinutes = _prefs.getInt('eta_remaining_minutes_$taskId');
    final savedStartTime = _prefs.getInt('eta_start_time_$taskId');
    final isActive = _prefs.getBool('eta_active_$taskId') ?? false;

    if (savedTaskId == taskId && savedRemainingMinutes != null &&
        savedStartTime != null && isActive) {

      final currentTime = DateTime.now().millisecondsSinceEpoch;
      final elapsedMinutes = ((currentTime - savedStartTime) / (1000 * 60)).floor();
      final newRemainingMinutes = savedRemainingMinutes - elapsedMinutes;

      if (newRemainingMinutes > 0) {
        maintenanceETA.value = newRemainingMinutes;
        isEtaTimerActive.value = true;
        _startEtaCountdown(taskId);
      } else {
        // Timer expired, clean up
        await _clearEtaTimer(taskId);
        maintenanceETA.value = 0;
        isEtaTimerActive.value = false;
      }
    }
  }

  Future<void> _clearEtaTimer(int taskId) async {
    await _prefs.remove('eta_task_id_$taskId');
    await _prefs.remove('eta_remaining_minutes_$taskId');
    await _prefs.remove('eta_start_time_$taskId');
    await _prefs.remove('eta_active_$taskId');
  }

  void _startEtaCountdown(int taskId) {
    _etaTimer?.cancel(); // Cancel any existing timer

    _etaTimer = Timer.periodic(Duration(minutes: 1), (timer) async {
      if (maintenanceETA.value > 0) {
        maintenanceETA.value--;

        // Save updated time to SharedPreferences
        await _saveEtaTimer(taskId, maintenanceETA.value);
        ...
        Get.snackbar(
          'ETA Completed',
          'The estimated cleaning time has been reached',
          snackPosition: SnackPosition.BOTTOM,
  ...
  Future<void> fetchTaskDetails(int scheduleId) async {
    try {
      ...
      if (taskData != null && taskData.isNotEmpty) {
        ...
        // Load any existing ETA timer for this task
        await _loadEtaTimer(scheduleId);
      }
    } catch (e) {
      errorMessage.value = e.toString();
      Get.snackbar(
  ...
  Future<void> fetchReportData(int scheduleId) async {
    try {
      ...
      if (response.success && response.data != null) {
        ...
        // Load ETA timer if task is in cleaning mode
        if (taskStatus.value == 'cleaning') {
          await _loadEtaTimer(scheduleId);
        }
      } else {
        throw Exception(response.errorMessage ?? 'Failed to fetch report data');
  ...
  Future<void> _updateTaskStatusToFinal(String finalStatus) async {
    try {
      ...
      // Stop and clear ETA timer when task is completed
      _etaTimer?.cancel();
      isEtaTimerActive.value = false;
      await _clearEtaTimer(selectedTaskId.value);
    ...
    try {
      ...
      if (taskStatus.value == 'pending') {
        ...
        if (response.success) {
          ...
          // Reset ETA to 120 minutes and start countdown
          maintenanceETA.value = 120;
          isEtaTimerActive.value = true;
          _startEtaCountdown(selectedTaskId.value);
  ...
  Map<String, int> getPanelCounts() {
    ...
    if (taskStatus.value == 'done') {
      ...
      final progress = 1 - (maintenanceETA.value / 120.0);
      completed = (total * progress).round();
    }
  ...
  // Helper method to format ETA display
  String get formattedETA {
    if (maintenanceETA.value <= 0) {
      return '0m';
    }

    final hours = maintenanceETA.value ~/ 60;
    final minutes = maintenanceETA.value % 60;
